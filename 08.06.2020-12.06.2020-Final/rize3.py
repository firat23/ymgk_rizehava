...
class Window(QtWidgets.QMainWindow):
     ...
     def  __init__(self, parent=None):
          ...
          loadUi("veri_bilimi.ui",self)
       
       

          self.dosyasec.clicked.connect(self.dosya_ac)
          self.egitmeyebasla.clicked.connect(self.islem)
     def dosya_ac(self):
          
          self.dosyaismi = QFileDialog.getOpenFileName(self,"Dosya Aç",os.getenv("HOME"))[0]
          

     def sinif(self,havakalite):
         if havakalite <= 70:
             return 1
         elif havakalite >70:
             return 2
     ...
     def islem(self):#Eğitmeye başla butonuna tıklandığında yapılacak işlemler
        
          haftaici=[]
          haftasonu=[]
          
          hava=pd.read_csv(self.dosyaismi)
          ...
          havakalite=gunler.max(axis=1).apply(self.sinif)
          ...
          for x in enumerate(gunler.iloc[:,-1]):
            ...
            if havakalite[sayi]!=1 and haftasonumu==False:
              artis=1
            elif havakalite[sayi]==1 and haftasonumu==False:
              artis=2
            elif havakalite[sayi]!=1 and haftasonumu==True:
              artis=3
            elif havakalite[sayi]==1 and haftasonumu==True:
              artis=4
